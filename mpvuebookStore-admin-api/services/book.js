    ...
    try {
      if (book instanceof Book) {
        ...
        if (result) {
          ...
          if (Number(result.updateType) === 1) {
            reject(new Error('默认图书不能编辑'))
          } else {
            delete model.createDt // 创建时间不能更新
            console.log(result.createUser,book.createUser)
